...
from ._test_data import generate_test_data
...
def report(ctx, in_dir, out_dir, pdf_path):
    ...
    try:
        ...
        # Load artifacts from output directory
        ...
        log.error(f"Missing required file: {e.filename}")
        if 'scores.csv' in str(e.filename) or 'selected_idx.txt' in str(e.filename) or 'x.npy' in str(e.filename):
            log.error("This file is generated by the 'detect' command. Please run it first.")
        elif 'x_cf.npy' in str(e.filename) or 'opt_hist.csv' in str(e.filename):
            log.error("This file is generated by the 'explain' command. Please run it first.")
        else:
            log.error("A required input file could not be found. Please check your --in and --out directories.")
        raise click.Abort()
    except Exception as e:
        log.exception(f"An unexpected error occurred while loading data: {e}", extra={"rich_traceback": True})
        log.error("One of the input files might be corrupted or in an unexpected format.")
        raise click.Abort()
...
@cli.command()
@click.option('--out', 'out_dir', required=True, type=click.Path(file_okay=False), help='Directory to save the generated test files')
def generate_test_data_cmd(out_dir):
    """
    Generates a full set of test data and a trained model in the specified directory.
    This allows for easy testing of the full detect->explain->report pipeline.
    """
    log.info(f"Generating test data in '{out_dir}'...")
    generate_test_data(out_dir)

# Add the new command to the cli group
cli.add_command(generate_test_data_cmd, name='generate-test-data')
